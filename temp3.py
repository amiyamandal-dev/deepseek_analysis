...
class DeepSeek:
    def __init__(self):
        self.system_prompt = '''<think>**Knowledge Integration Protocol**
1. Context Evaluation:
   - Thoroughly analyze the provided context for direct relevance to the question
   - Determine if context contains specific information that substantively addresses the query

2. Information Synthesis:
   [When Context is Relevant]
   - Prioritize context-based information
   - Extract and integrate key details while maintaining factual accuracy
   - Explicitly connect context evidence to question resolution

   [When Context is Irrelevant]
   - Disregard provided context completely
   - Engage internal knowledge base
   - Acknowledge context absence in your response formulation

3. Response Generation:
   - Provide a clear, structured answer with coherent reasoning
   - Maintain strict factual accuracy regardless of knowledge source
   - If using context, include relevant verbatim excerpts when appropriate
   - For numerical/statistical questions, specify precision level
   - For comparative questions, establish clear evaluation criteria

4. Quality Assurance:
   - Verify answer consistency with best available evidence
   - Flag any potential contradictions between sources
   - Confidently state "I don't know" for unverifiable information
</think>'''

    def generate(self, model: str, prompt: str) -> Optional[str]:
        try:
            # Possibly: openai_python_client.api_base = "https://api.deepseek.com"
            r = ollama.generate(model='deepseek-r1:14b', prompt=f"{self.system_prompt}\n\n Content{prompt}")
...
class Judge:
    ...
    def evaluate(self, question: str, correct: str, response: str) -> int:
        ...
        if "</think>" in correct:
            correct = correct.split("</think>")[-1].strip()
